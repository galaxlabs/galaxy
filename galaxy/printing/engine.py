...
from galaxy.printing.builder import get_field_variables
...
def _render_html(doctype: str, docname: str, format_name: str = "Standard", letterhead: str | None = None) -> str:
    ...
    font_family = pf.get("font_family", "'Segoe UI', Tahoma, 'Noto Naskh Arabic', sans-serif")
    lh_html = ""
    if letterhead:
        lh = _get_letterhead(letterhead)
        if lh:
            lh_html = lh.get("letterhead_html", "")
    field_vars = get_field_variables(fields, doc)
    ctx = {
        "doc": doc,
        "doctype": doctype,
        "docname": docname,
